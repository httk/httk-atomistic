...
3. On PyPI, add a pending GitHub Trusted Publisher with these values:
   ...
   - PyPI project name: `httk-atomistic`
   - Owner: `httk`
   - Repository: `httk-atomistic`
...
```console
python -m venv /tmp/httk-atomistic-test
/tmp/httk-atomistic-test/bin/python -m pip install \
  --index-url https://test.pypi.org/simple/ \
  --extra-index-url https://pypi.org/simple/ httk-atomistic==0.1.0
/tmp/httk-atomistic-test/bin/python -c "import httk.atomistic"
```

Replace `0.1.0` with the version being tested. Unlike `httk-core`, `httk-atomistic`
...
4. Verify the release from a fresh environment with `pip install httk-atomistic`.
