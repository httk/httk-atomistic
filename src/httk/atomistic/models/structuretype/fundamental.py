...
class FundamentalDomainTemplate(StructuretypeBackend):
    ...
    def __init__(
        ...
        require_full_periodicity(self._cell, "FundamentalDomainTemplate")
        ...
        if not self._spacegroup.is_standard_setting:
            raise ValueError(
                f"FundamentalDomainTemplate records Wyckoff data in the IT standard setting, but was given "
        ...
        if len({value.name for value in norm_species}) != len(norm_species):
            raise ValueError("FundamentalDomainTemplate species names must be unique")
        if any(not is_dummy_species(value) for value in norm_species):
            raise ValueError("FundamentalDomainTemplate species must be dummy species")
        known = {value.name for value in norm_species}
        for site in raw_sites:
            if site.representative is not None:
                raise ValueError("FundamentalDomainTemplate WyckoffSite representative is not supported")
            if site.moment is not None:
                raise ValueError("FundamentalDomainTemplate WyckoffSite moment is not supported")
            if site.species not in known:
                raise ValueError(
                    f"FundamentalDomainTemplate WyckoffSite references unknown species name: {site.species!r}"
                )
        ...
        if {value.name for value in norm_species} != expected:
            raise ValueError("FundamentalDomainTemplate species labels must be consecutive anonymous symbols from 'A'")
    ...
    def __eq__(self, other: object) -> bool:
        if not isinstance(other, FundamentalDomainTemplate):
    ...
    def __repr__(self) -> str:
        return f"FundamentalDomainTemplate({self._spacegroup.setting!r}, wyckoff_sites={self._wyckoff_sites!r})"


class ASUTemplate(FundamentalDomainTemplate):
    ...
    :class:`FundamentalDomainTemplate` whose Wyckoff sites are known to span a real
