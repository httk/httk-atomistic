"""The immutable assigned geometrical-class crystallotype value."""
...
def _protostructure_from_representative(representative: FundamentalDomainStructure) -> Protostructure:
    ...
    if not isinstance(representative, FundamentalDomainStructure):
        raise TypeError("Crystallotype representative must be a FundamentalDomainStructure")
    if not representative.spacegroup.is_standard_setting:
        raise ValueError("Crystallotype representative must record Wyckoff data in the IT standard setting")
    if not representative.transform.is_identity():
        raise ValueError("Crystallotype representative must use an identity setting transform")
    if representative.assemblies is not None:
        raise ValueError("Crystallotype representative cannot carry assemblies")
    if any(site.moment is not None for site in representative.wyckoff_sites):
        raise ValueError("Crystallotype representative cannot carry site moments")
...
class Crystallotype(CrystallotypeBackend):
    ...
    ``Crystallotype`` is the assigned-species, geometrical-class cell of the
    ...
    kind: ClassVar[str] = "crystallotype"
    ...
    def __init__(
        ...
        *,
        ...
        if representative is None and discriminator is None:
            raise ValueError("Crystallotype requires at least one of representative or discriminator")
        if discriminator is not None and (not isinstance(discriminator, str) or not discriminator):
            raise ValueError("Crystallotype discriminator must be a non-empty string when given")
        ...
        if protostructure is not None:
            ...
            if derived is not None and held != derived:
                raise ValueError("Crystallotype protostructure disagrees with its representative's occupations")
            result = held
        elif derived is not None:
            result = derived
        else:
            raise ValueError("Crystallotype needs a protostructure when only a discriminator is given")
    ...
    def __eq__(self, other: object) -> bool:
        if not isinstance(other, Crystallotype):
    ...
    __hash__ = None  # type: ignore[assignment]  # representative is unhashable; a crystallotype is a value key by equality only
    ...
    def __repr__(self) -> str:
        ...
        return f"Crystallotype({', '.join(parts)})"
