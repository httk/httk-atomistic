...
class ChromaformulaView(ChemicalFormulaViewBase, Chromaformula):
    r"""Present a complete composition as an eager canonical anonymous formula.

    The canonical class name is ``ChromaformulaView``; the legacy
    ...
    def __new__(cls, obj: "ChemicalFormulaLike", **hints: Any) -> Self:
        ...
        if backend.is_anonymous:
            if type(backend) is Chromaformula:
                text = str(backend)
