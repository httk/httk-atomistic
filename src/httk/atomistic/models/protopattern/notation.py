...
def render_protopattern_label(spacegroup: Spacegroup, occupations: Sequence[tuple[str, str]]) -> str:
...
def render_protostructure_label(spacegroup: Spacegroup, occupations: Sequence[tuple[str, str]]) -> str:
    ...
    unsuffixed prefix equals the protopattern label of the erased pattern; the suffix lists
...
def parse_protopattern_label(text: str) -> "Protopattern":
    ...
    :param text: The protopattern label to parse.
    :return: The parsed :class:`~httk.atomistic.models.protopattern.protopattern.Protopattern`.
    :raises ValueError: If ``text`` is not a canonical protopattern label.
    """
    from httk.atomistic.models.protopattern.protopattern import Protopattern

    main, fields, names = _split_label(text)
    if names is not None:
        raise ValueError("a protopattern label carries no ':' species suffix")
    ...
    value = Protopattern(it_number, occupations)
    if render_protopattern_label(value.spacegroup, [(o.wyckoff, o.label) for o in value.occupations]) != main:
        raise ValueError(f"{text!r} is not a canonical protopattern label")
    return value
...
def parse_protostructure_label(text: str) -> "Protostructure":
    ...
    The unsuffixed part is validated as for a protopattern label; each ``:`` name must be a
...
def try_parse_protopattern(text: str) -> "Protopattern | None":
    """Return the parsed protopattern, or ``None`` when *text* is not a canonical one.

    :param text: The label text to test.
    :return: The parsed protopattern, or ``None`` for a non-label string.
    """
    if not isinstance(text, str) or ":" in text:
        return None
    try:
        return parse_protopattern_label(text)
    except ValueError:
        return None
