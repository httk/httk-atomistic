...
class ProtopatternLabelString(ProtopatternBackend):
    r"""Wrap a canonical protopattern label held as a plain string.

    The string must be a canonical unsuffixed protopattern label (no ``:`` species
    suffix); anything else is declined. Parsing is eager, so the wrapped value is
    validated at adoption.

    :param obj: The canonical protopattern label text.
    ...
    def _backend_adopt(cls, obj: Any, **hints: Any) -> Self | None:
        r"""Adopt a canonical protopattern label string.
        ...
        if not isinstance(obj, str):
            return None
        if try_parse_protopattern(obj) is None:
            return None
        return cls(obj, **hints)

    def __init__(self, obj: str, **hints: Any) -> None:
        self._raw = obj
        parsed = try_parse_protopattern(obj)
