...
class FundamentalDomainStructure(StructureSemanticsMixin, StructureBackend):
    ...
    :param _deduplicate_redundant_sites: Internal file-ingestion mode for accepting identical
        redundant listings; storage-backed structures leave this disabled.
    """
    ...
    def __init__(
        ...
        *,
        ...
        _deduplicate_redundant_sites: bool = False,
    ) -> None:
        ...
        self._deduplicate_redundant_sites = _deduplicate_redundant_sites
    ...
    def _expansion(self) -> tuple[FracVector, tuple[str, ...], tuple[int, ...]]:
        ...
        point cannot silently produce a doubled atom. Redundant listings with identical
        species are accepted in the file-ingestion mode; coincident listings with different
        species always raise ``ValueError``. Canonical storage records use strict mode.
        """
        ...
        counts: list[int] = []
        species_by_name = {species.name: species for species in self._species}
            ...
            for standard_point in position.coordinates(site.free_params):
                ...
                for coset in cosets:
                    ...
                    if previous is not None:
                        ...
                        if species_by_name[previous_name] != species_by_name[site.species]:
                            raise ValueError(
                                f"{site!r} coincides with {previous_site!r} at {key} but has a different species"
                            )
                        if previous_site is not site and not self._deduplicate_redundant_sites:
                            raise ValueError(
                                f"{site!r} has an orbit that contributes no new sites; it duplicates an earlier "
                                "site's orbit, so this is not a valid fundamental domain"
                            )
                        continue
                    seen[key] = (site.species, site)
