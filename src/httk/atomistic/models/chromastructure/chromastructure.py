...
class Chromastructure(ChromastructureBackend):
    """Store a unit cell whose site identities are consecutive dummy labels.

    ``Chromastructure`` is the anonymous-species, exact-geometry cell of the
    ...
    def __init__(
        ...
        if species_at_sites is None:
            raise TypeError("Chromastructure species_at_sites is required")
        ...
        if len({value.name for value in norm_species}) != len(norm_species):
            raise ValueError("Chromastructure species names must be unique")
        if any(not is_dummy_species(value) for value in norm_species):
            raise ValueError("Chromastructure species must be dummy species")
        known = {value.name for value in norm_species}
        for label in norm_species_at_sites:
            if label not in known:
                raise ValueError(f"Chromastructure species_at_sites references unknown species name: {label!r}")
        expected = {anonymous_symbol(index) for index in range(len(norm_species))}
        if {value.name for value in norm_species} != expected:
            raise ValueError("Chromastructure species labels must be consecutive anonymous symbols from 'A'")
    ...
    def __eq__(self, other: object) -> bool:
        if not isinstance(other, Chromastructure):
    ...
    def __repr__(self) -> str:
        return (
            f"Chromastructure(cell={self._cell!r}, sites={self._sites!r}, species_at_sites={self._species_at_sites!r})"
        )
