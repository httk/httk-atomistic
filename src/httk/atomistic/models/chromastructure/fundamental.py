...
from httk.atomistic.models.chromastructure.anonymize import dummy_species, is_dummy_species
...
class FundamentalDomainPattern(ChromastructureBackend):
    """Store a standard-setting fundamental domain with dummy species labels.

    ``FundamentalDomainPattern`` is the fundamental-domain member of the ``Chromastructure``
    ...
    def __init__(
        ...
        require_full_periodicity(self._cell, "FundamentalDomainPattern")
        ...
        if not self._spacegroup.is_standard_setting:
            raise ValueError(
                f"FundamentalDomainPattern records Wyckoff data in the IT standard setting, but was given "
        ...
        if len({value.name for value in norm_species}) != len(norm_species):
            raise ValueError("FundamentalDomainPattern species names must be unique")
        if any(not is_dummy_species(value) for value in norm_species):
            raise ValueError("FundamentalDomainPattern species must be dummy species")
        known = {value.name for value in norm_species}
        for site in raw_sites:
            if site.representative is not None:
                raise ValueError("FundamentalDomainPattern WyckoffSite representative is not supported")
            if site.moment is not None:
                raise ValueError("FundamentalDomainPattern WyckoffSite moment is not supported")
            if site.species not in known:
                raise ValueError(
                    f"FundamentalDomainPattern WyckoffSite references unknown species name: {site.species!r}"
                )
        ...
        if {value.name for value in norm_species} != expected:
            raise ValueError("FundamentalDomainPattern species labels must be consecutive anonymous symbols from 'A'")
    ...
    def protochroma(self) -> "Protochroma":
        """Return the anonymous protochroma this fundamental domain folds to.

        The protochroma keeps this domain's occupied Wyckoff letters and their dummy-species
        class partition; the exact geometry is discarded. The protochroma's constructor
        re-canonicalizes the class labels by the pinned group-ordering rule.

        :return: The folded :class:`~httk.atomistic.models.protochroma.protochroma.Protochroma`.
        """
        ...
        return Protochroma(
            self._spacegroup,
            [ProtochromaOccupation(site.wyckoff, site.species) for site in self._wyckoff_sites],
        )
    ...
    def __repr__(self) -> str:
        return f"FundamentalDomainPattern({self._spacegroup.setting!r}, wyckoff_sites={self._wyckoff_sites!r})"


class ASUPattern(FundamentalDomainPattern):
    """Assert that a fundamental-domain pattern is a true asymmetric unit.

    Mirror :class:`~httk.atomistic.models.structure.asu.ASUStructure`: mark a
    :class:`FundamentalDomainPattern` whose Wyckoff sites are known to span a real
    asymmetric unit rather than merely a fundamental domain. The family exposes no
    ``site_coordinate_span`` hook, so this marker adds no state and inherits the
    fundamental-domain equality and representation.
    """
