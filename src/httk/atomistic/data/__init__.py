...
Four datasets ship with *httk-atomistic*, all read through
:class:`~httk.core.DatasetLoader` and all licensed CC BY 4.0 (see the adjacent
...
``spacegroup_subgroups.json.gz``
    Per-IT-number Bärnighausen subgroup transformations and continuous-normalizer bases.
...
def _subgroups() -> DatasetLoader:
    ...
    return DatasetLoader(
        "httk.atomistic.spacegroup_subgroups",
        _resource_path("spacegroup_subgroups.json.gz"),
    )
...
def spacegroup_subgroup_record(it_number: int) -> dict[str, Any]:
    ...
    """
    index = _lookup_index(_subgroups(), "spacegroup_subgroups", "index_it_number_to_spacegroup_subgroups")
    try:
        position = index[str(int(it_number))]
    except KeyError:
        raise KeyError(f"no space-group subgroup record with IT number {it_number!r}") from None
    return _subgroups().data.spacegroup_subgroups[position]


def affine_normalizer_coset_record(hall_entry: str) -> dict[str, Any]:
