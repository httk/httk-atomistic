...
def test_hexagonal_end_to_end_is_exact() -> None:
    ...
    assert cell.lengths == (SurdVector(3), SurdVector(3), SurdVector(5))
...
def test_rhombohedral_60_exact_reconstruction() -> None:
    ...
    assert cell.lengths == (SurdVector(1), SurdVector(1), SurdVector(1))
...
def test_scale_factors_out_exactly() -> None:
    ...
    unscaled = SurdVector._from_scalar_grid(
        [
            [one, zero, zero],
            [SurdVector(F(-1, 2))._as_scalar(), sqrt3_over_2, zero],
            [zero, zero, SurdVector(F(5, 3))._as_scalar()],
        ],
        (3, 3),
    )
    ...
    assert scaled.scale == SurdVector(3)
    # Volume scales as scale**3; angles are scale-independent.
    unit = Cell(unscaled)
    assert scaled.volume == (unit.volume * SurdVector(27))._as_scalar()
...
def test_exact_cartesian_and_metric_bond_length() -> None:
    ...
    cart_diff = SurdVector(diff) * cell.basis
    lsq_cartesian = cart_diff.lengthsqr()
    metric = cell.metric()
    lsq_metric = (SurdVector(diff) * metric).dot(SurdVector(diff))
...
def test_vectorlike_inputs_fraction_string_fracvector() -> None:
    ...
    assert frac_cell.basis == SurdVector([[F(1, 2), 0, 0], [0, F(3, 2), 0], [0, 0, 1]])
    # Rational strings.
    str_sites = Sites([["1/3", "1/3", "1/3"]])
    assert str_sites.reduced_coords == FracVector([[F(1, 3), F(1, 3), F(1, 3)]])
    # FracVector directly.
    fv_cell = Cell(FracVector(identity))
    assert fv_cell.basis == SurdVector(identity)
    # A UnitcellStructure accepts these forms.
    structure = UnitcellStructure(
        cell=[["1/3", 0, 0], [0, 1, 0], [0, 0, 1]],
        sites=FracVector([[F(1, 4), F(1, 4), F(1, 4)]]),
        species=[{"name": "Si", "chemical_symbols": ["Si"], "concentration": [1.0]}],
        species_at_sites=["Si"],
    )
    assert structure.cell.basis.coefficient(1)[0][0] == FracVector(F(1, 3))
    assert structure.sites.reduced_coords == FracVector([[F(1, 4), F(1, 4), F(1, 4)]])


def test_vectorlike_numpy_input() -> None:
    numpy = pytest.importorskip("numpy")
    cell = Cell(numpy.array([[2.0, 0.0, 0.0], [0.0, 2.0, 0.0], [0.0, 0.0, 2.0]]))
    assert cell.basis == SurdVector([[2, 0, 0], [0, 2, 0], [0, 0, 2]])
    sites = Sites(numpy.array([[0.0, 0.0, 0.0], [0.5, 0.5, 0.5]]))
    assert sites.reduced_coords == FracVector([[0, 0, 0], [F(1, 2), F(1, 2), F(1, 2)]])
