"""Golden and round-trip tests for httk protochroma/protostructure label notation."""

import pytest

from httk.atomistic import Protochroma, Spacegroup
from httk.atomistic.models.protochroma.notation import (
    parse_protochroma_label,
    parse_protostructure_label,
    pearson_symbol,
    render_aflow_label,
    render_protochroma_label,
    render_protostructure_label,
)


def test_rocksalt_protochroma_label() -> None:
    pattern = Protochroma(225, [("a", "A"), ("b", "B")])
    assert str(pattern.label) == "AB_cF8_225_a_b"


def test_calcite_protochroma_label() -> None:
    pattern = Protochroma(167, [("a", "A"), ("b", "B"), ("e", "C")])
...
def test_special_27th_letter_round_trips_as_uppercase_a() -> None:
    pattern = Protochroma(47, [("α", "A")])
    assert str(pattern.label) == "A_oP8_47_A"
    assert parse_protochroma_label("A_oP8_47_A") == pattern


def test_repeated_letter_group_pins() -> None:
    pattern = Protochroma(47, [("i", "A"), ("i", "A")])
    assert str(pattern.label) == "A_oP4_47_2i"
    assert parse_protochroma_label("A_oP4_47_2i") == pattern
...
def test_repeated_special_letter_group_pins() -> None:
    ...
    pattern = Protochroma(47, [("α", "A"), ("α", "A")])
    assert str(pattern.label) == "A_oP16_47_2A"
    assert parse_protochroma_label("A_oP16_47_2A") == pattern
...
def test_non_rhombohedral_pearson_is_not_divided(it_number: int, occupations: list, expected: str) -> None:
    pattern = Protochroma(it_number, occupations)
    assert str(pattern.label) == expected
    assert parse_protochroma_label(expected) == pattern
...
def test_render_parse_render_loop_over_valid_patterns() -> None:
    cases = [
        Protochroma(225, [("a", "A"), ("b", "B")]),
        Protochroma(167, [("a", "A"), ("b", "B"), ("e", "C")]),
        Protochroma(47, [("α", "A")]),
        Protochroma(47, [("i", "A"), ("i", "A")]),
        Protochroma(1, [("a", "A")]),
        Protochroma(225, [("a", "A"), ("b", "A"), ("c", "B")]),
    ]
    for pattern in cases:
        text = render_protochroma_label(
            pattern.spacegroup, [(o.wyckoff, o.label) for o in pattern.occupations]
        )
        assert parse_protochroma_label(text) == pattern
        again = parse_protochroma_label(text)
        assert render_protochroma_label(again.spacegroup, [(o.wyckoff, o.label) for o in again.occupations]) == text
