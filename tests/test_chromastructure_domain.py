"""Tests for standard-setting fundamental-domain patterns (recognition, laziness, pickle)."""
...
def test_prototype_standard_setting_and_canonical_site_order() -> None:
    ...
    first = FundamentalDomainPattern(CELL, 225, (WyckoffSite("b", EMPTY, "B"), WyckoffSite("a", EMPTY, "A")), species)
    second = FundamentalDomainPattern(CELL, 225, tuple(reversed(first.wyckoff_sites)), species)
    assert first == second
    with pytest.raises(ValueError, match="standard setting"):
        FundamentalDomainPattern(
            CELL, Spacegroup.from_setting("15:c1"), (WyckoffSite("e", FracVector([1, 3]), "A"),), (species[0],)
        )
...
def test_prototype_rejects_representatives_moments_and_bad_free_count() -> None:
    ...
    with pytest.raises(ValueError, match="free parameter"):
        FundamentalDomainPattern(CELL, 225, (WyckoffSite("a", FracVector([1]), "A"),), (dummy("A"),))
...
def test_exact_asu_path_and_expansion() -> None:
    asu = _rocksalt_asu()
    prototype = FundamentalDomainPatternView(asu)
    assert FundamentalDomainPatternView(asu, kind="structure") == prototype
    with pytest.raises(TypeError):
        FundamentalDomainPatternView(asu, kind="bogus")
    ...
    assert isinstance(ChromaformulaView(prototype), str)
    with pytest.raises(ValueError):
        FundamentalDomainPatternView(asu, tolerance=1e-5)
    with pytest.raises(ValueError):
...
def test_prototype_view_rewrap_rejects_arguments() -> None:
    view = FundamentalDomainPatternView(_rocksalt_asu())
    with pytest.raises(ValueError):
        FundamentalDomainPatternView(view, tolerance=0.123)
    with pytest.raises(ValueError):
        FundamentalDomainPatternView(view, setting="bogus")
    with pytest.raises(ValueError):
        FundamentalDomainPatternView(view, kind="bogus")
...
def test_anonymous_prototype_source_uses_original_labels() -> None:
    ...
    prototype = FundamentalDomainPatternView(anonymous)
...
def test_formula_views_reduce_non_coprime_amounts_and_remain_parseable() -> None:
    asu = _rocksalt_asu()
    prototype = FundamentalDomainPatternView(asu)
    assert Chromaformula(str(prototype.anonymous_formula)) == "AB"

    real_structure = UnitcellStructureView(asu)
    assert Chromaformula(str(ChromaformulaView(real_structure))) == "AB"
...
def test_recognition_path_is_spglib_gated() -> None:
    ...
    assert FundamentalDomainPatternView(unitcell) == FundamentalDomainPatternView(asu)
...
def test_prototype_view_resolves_nested_asu_source_once_across_value_operations() -> None:
    ...
    view = FundamentalDomainPatternView(asu)
...
def test_prototype_view_retains_tolerance_and_denominator_and_resolves_source_once(monkeypatch) -> None:
    module = __import__("httk.atomistic.models.chromastructure.fundamental_view", fromlist=["conventional_cell"])
    source = CountingStructureResolver(_rocksalt_unitcell())
    ...
    view = FundamentalDomainPatternView(source, tolerance=0.125, limit_denominator=17)
    assert source.resolve_calls == 0
    _ = view.spacegroup
    assert source.resolve_calls == 1
...
def test_prototype_view_unsupported_data_fails_atomically_on_first_access() -> None:
    ...
    view = FundamentalDomainPatternView(mixed)
...
def test_prototype_view_pickle_preserves_unresolved_and_resolved_states() -> None:
    source = CountingStructureResolver(_rocksalt_unitcell())
    unresolved = FundamentalDomainPatternView(ASUStructureView(source, setting=Spacegroup.standard(1)))
    ...
    source = CountingStructureResolver(_rocksalt_unitcell())
    resolved = FundamentalDomainPatternView(ASUStructureView(source, setting=Spacegroup.standard(1)))
...
def test_prototype_view_native_unview_preserves_identity() -> None:
    native = FundamentalDomainPattern(
        CELL,
        225,
        (WyckoffSite("a", EMPTY, "A"),),
        (dummy("A"),),
    )
    assert FundamentalDomainPatternView(native).unview() is native
...
def test_prototype_datastream_path_is_not_parsed_at_construction(tmp_path, monkeypatch) -> None:
    ...
    view = FundamentalDomainPatternView(str(path))
    assert calls == 0
