...
def test_base_only_and_representative_only_are_valid() -> None:
    ...
    explicit = BareProtostructure(225, [("a", representative.species[0]), ("b", representative.species[1])])
    assert derived.spacegroup == explicit.spacegroup
    assert derived.occupations == explicit.occupations
    assert not derived.similar(explicit, 0.0)
...
def test_representative_validation_and_mismatch() -> None:
    ...
    with pytest.raises(ValueError, match="molecular"):
        BareProtostructureView(molecular).unview()
...
def test_similar_optional_fields_and_delta_validation() -> None:
    one = Protostructure(225, [("a", "Na"), ("b", "Cl")], discriminator="001")
    two = Protostructure(225, [("a", "Na"), ("b", "Cl")], discriminator="002")
    assert one.similar(one, 0.0)
    assert not one.similar(two, 0.0)
    assert not one.similar(BareProtostructure(225, [("a", "Na"), ("b", "Cl")]), 0.0)
...
def test_construction_promotes_inputs_and_canonicalizes() -> None:
    first = BareProtostructure(225, [("a", "Na"), ("b", "Cl")])
    second = BareProtostructure(Spacegroup.standard(225), [("b", "Cl"), ("a", "Na")])
    assert first == second
    ...
    with pytest.raises(ValueError, match="standard setting"):
        BareProtostructure(Spacegroup.from_setting("15:c1"), [("e", "Na")])
    with pytest.raises(ValueError, match="non-empty"):
        BareProtostructure(225, [])
    with pytest.raises(ValueError, match="no Wyckoff letter"):
        BareProtostructure(225, [("zz", "Na")])
...
def test_disorder_and_vacancy_project_with_tabulated_multiplicity() -> None:
    ...
    value = BareProtostructure(221, [("a", mixed), ("b", vacancy)])
...
def test_exact_paths_preserve_species_and_match_structure_formula() -> None:
    asu = _rocksalt_asu()
    value = BareProtostructureView(asu)
    ...
    other = BareProtostructureView(FundamentalDomainStructure(CELL, 225, asu.wyckoff_sites, asu.species))
    assert other.spacegroup == value.spacegroup
    assert other.occupations == value.occupations
    assert not hasattr(other, "representative")
    assert other == value
    with pytest.raises(ValueError):
        BareProtostructureView(asu, tolerance=1e-5)
    assert BareProtostructureView(value) is value
    with pytest.raises(ValueError):
        BareProtostructureView(value, tolerance=1e-5)
...
def test_protostructure_views_and_labels_preserve_optional_identity() -> None:
    ...
    recognized = RecognizedBareProtostructure(representative)
    view = BareProtostructureView(recognized)
    assert view._resolved_protostructure is None
    base = view.unview()
    assert not hasattr(base, "representative") and not hasattr(base, "discriminator")
    assert BareProtostructureLabel(recognized).unview() == base
...
def test_transform_scaled_source_uses_standard_conventional_scale() -> None:
    ...
    value = BareProtostructureView(asu)
    direct = BareProtostructure(166, [("a", bi), ("b", oxygen)])
    ...
    assert not hasattr(value, "representative")
    assert value == direct
...
def test_recognition_probe_re_raises_errors_from_a_matched_structure_backend() -> None:
    ...
    with pytest.raises(TypeError, match="SpeciesBackend"):
        BareProtostructureView(raw)


def test_recognition_path_and_rejections() -> None:
    pytest.importorskip("spglib")
    asu = _rocksalt_asu()
    assert BareProtostructureView(UnitcellStructureView(asu)) == BareProtostructureView(asu)
    with pytest.raises(TypeError):
        BareProtostructureView(asu, kind="bogus")

    anonymous = AnonymousStructure(CELL, [[0, 0, 0]], species_at_sites=("A",))
    with pytest.raises(TypeError, match="dummy species"):
        BareProtostructureView(anonymous)
    with pytest.raises(TypeError):
        UnitcellStructureView(BareProtostructure(225, [("a", "Na")]))
    assert BarePrototypeView(BareProtostructure(225, [("a", "Na")])).unview().occupations[0].label == "A"
    ...
    view = BareProtostructureView(structure)
    ...
    view = BareProtostructureView(chemical)
    ...
    view = BareProtostructureView(moments)
...
def test_protostructure_view_resolves_counting_source_once_across_value_operations() -> None:
    ...
    view = BareProtostructureView(source, setting=Spacegroup.standard(1))
...
def test_protostructure_view_rejects_options_at_deferred_asu_boundary() -> None:
    asu = _rocksalt_asu()
    backend = RecognizedBareProtostructure(asu, setting=Spacegroup.standard(225))
    ...
    with pytest.raises(ValueError, match="recognition arguments cannot be used with an existing ASU"):
        BareProtostructureView(outer, tolerance=1e-5)
    assert source.resolve_calls == 0


def test_protostructure_view_retains_and_passes_all_recognition_options(monkeypatch: pytest.MonkeyPatch) -> None:
    module = __import__("httk.atomistic.models.bareprotostructure.recognized", fromlist=["recognize_asu"])
...
def test_protostructure_unsupported_data_fails_atomically_on_first_access() -> None:
    ...
    view = BareProtostructureView(unknown)
...
def test_protostructure_view_pickle_preserves_unresolved_and_resolved_states() -> None:
    unresolved = BareProtostructureView(CountingStructureResolver(_rocksalt_unitcell()), setting=Spacegroup.standard(1))
    ...
    resolved = BareProtostructureView(CountingStructureResolver(_rocksalt_unitcell()), setting=Spacegroup.standard(1))
...
def test_protostructure_view_accepts_custom_backend_and_native_identity() -> None:
    ...
    view = BareProtostructureView(custom)
    assert custom.spacegroup_calls == 0
    assert view.unwrap() is custom
    assert custom.spacegroup_calls == 0
    value = view.unview()
    assert type(value) is BareProtostructure
    assert custom.spacegroup_calls == 1
    assert custom.occupations_calls == 1

    native = BareProtostructure(225, [("a", "Na")])
    assert BareProtostructureView(native).unview() is native
...
def test_protostructure_label_round_trips_element_pure_value() -> None:
    ...
    calcite = BareProtostructure(167, pure)
    assert BareProtostructureView(str(calcite.label)) == calcite


def test_protostructure_label_string_dispatch() -> None:
    view = BareProtostructureView("AB_cF8_225_a_b:Na-Cl")
    assert view.spacegroup == Spacegroup.standard(225)
    assert str(BareProtostructureView("AB_cF8_225_a_b:Na-Cl").label) == "AB_cF8_225_a_b:Na-Cl"
...
def test_protostructure_datastream_path_is_not_parsed_at_construction(tmp_path, monkeypatch) -> None:
    ...
    view = BareProtostructureView(str(path), setting=Spacegroup.standard(1))
    assert calls == 0
...
def test_protostructure_view_resolves_storage_record() -> None:
    ...
    view = BareProtostructureView(BareProtostructure(225, [("a", "Na")]))
    assert resolve_storage_record(view) is BareProtostructureRecord


def test_similar_is_available_on_every_protostructure_backend() -> None:
    value = Protostructure(representative=_rocksalt_asu())
    view = ProtostructureView(value)
    generic = IdentityCarryingProtostructureBackend(value)
    assert view.similar(value, 0.0)
    assert generic.similar(view, 0.0)
    assert ProtostructureLabel(value).unview().similar(value, 0.0)
