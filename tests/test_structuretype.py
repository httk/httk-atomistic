"""Tests for the structuretype family."""
...
def test_transitional_aliases_are_the_canonical_classes() -> None:
    assert AnonymousStructure is Structuretype
    assert AnonymousStructureView is StructuretypeView


def test_crystaltemplate_name_is_fully_retired() -> None:
    assert not hasattr(atomistic, "CrystalTemplate")
    assert not hasattr(atomistic, "CrystalTemplateView")
    with pytest.raises(ModuleNotFoundError):
        importlib.import_module("httk.atomistic.models.crystaltemplate")
...
def test_anonymous_structure_infers_species_and_rejects_bad_labels() -> None:
    value = Structuretype(CELL, SITES, species_at_sites=("A", "B"))
    assert tuple(species.name for species in value.species) == ("A", "B")
    with pytest.raises(ValueError, match="consecutive"):
        Structuretype(CELL, [[0, 0, 0]], species_at_sites=("B",), species=(dummy_species("B"),))
    with pytest.raises(ValueError, match="unknown"):
        Structuretype(CELL, [[0, 0, 0]], species=(dummy_species("A"),), species_at_sites=("B",))
    with pytest.raises(ValueError, match="dummy"):
        Structuretype(CELL, [[0, 0, 0]], species=(Species("A", ("Na",), (1,)),), species_at_sites=("A",))
...
def test_canonical_assignment_and_canonicality() -> None:
    ...
    canonical = Structuretype(CELL, [[0, 0, 0], [0, 0, 0], [0, 0, 0]], species_at_sites=("A", "A", "B"))
    swapped = Structuretype(CELL, [[0, 0, 0], [0, 0, 0], [0, 0, 0]], species_at_sites=("A", "B", "B"))
...
def test_anonymous_view_is_canonical_lazy_and_preserves_formula() -> None:
    ...
    view = StructuretypeView(structure)
...
def test_anonymous_structures_are_not_structure_like() -> None:
    value = Structuretype(CELL, [[0, 0, 0]], species_at_sites=("A",))
...
def test_anonymization_ignores_implicit_atoms_attachments_and_duplicate_element_names() -> None:
    ...
    with pytest.raises(TypeError):
        StructuretypeView(structure, kind="bogus")
...
def test_matched_numeric_structure_re_raises_species_backend_errors() -> None:
    ...
    with pytest.raises(TypeError, match="SpeciesBackend"):
        StructuretypeView(NumericObject())
