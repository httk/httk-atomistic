"""Tests for the crystal-pattern family."""
...
def test_anonymous_structure_infers_species_and_rejects_bad_labels() -> None:
    value = CrystalPattern(CELL, SITES, species_at_sites=("A", "B"))
    assert tuple(species.name for species in value.species) == ("A", "B")
    with pytest.raises(ValueError, match="consecutive"):
        CrystalPattern(CELL, [[0, 0, 0]], species_at_sites=("B",), species=(dummy_species("B"),))
    with pytest.raises(ValueError, match="unknown"):
        CrystalPattern(CELL, [[0, 0, 0]], species=(dummy_species("A"),), species_at_sites=("B",))
    with pytest.raises(ValueError, match="dummy"):
        CrystalPattern(CELL, [[0, 0, 0]], species=(Species("A", ("Na",), (1,)),), species_at_sites=("A",))
...
def test_canonical_assignment_and_canonicality() -> None:
    ...
    canonical = CrystalPattern(CELL, [[0, 0, 0], [0, 0, 0], [0, 0, 0]], species_at_sites=("A", "A", "B"))
    swapped = CrystalPattern(CELL, [[0, 0, 0], [0, 0, 0], [0, 0, 0]], species_at_sites=("A", "B", "B"))
...
def test_anonymous_view_is_canonical_lazy_and_preserves_formula() -> None:
    ...
    view = CrystalPatternView(structure)
...
def test_anonymous_structures_are_not_structure_like() -> None:
    value = CrystalPattern(CELL, [[0, 0, 0]], species_at_sites=("A",))
...
def test_anonymization_rejects_unused_species_on_derivation_and_contradictory_kind() -> None:
    ...
    with pytest.raises(TypeError):
        CrystalPatternView(structure, kind="bogus")
...
def test_matched_numeric_structure_re_raises_species_backend_errors() -> None:
    ...
    with pytest.raises(TypeError, match="SpeciesBackend"):
        CrystalPatternView(NumericObject())
