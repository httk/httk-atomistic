"""Tests for standard-setting fundamental-domain templates (recognition, laziness, pickle)."""
...
def test_prototype_standard_setting_and_canonical_site_order() -> None:
    ...
    first = FundamentalDomainTemplate(CELL, 225, (WyckoffSite("b", EMPTY, "B"), WyckoffSite("a", EMPTY, "A")), species)
    second = FundamentalDomainTemplate(CELL, 225, tuple(reversed(first.wyckoff_sites)), species)
    assert first == second
...
def test_prototype_rejects_representatives_moments_and_bad_free_count() -> None:
    ...
    with pytest.raises(ValueError, match="free parameter"):
        FundamentalDomainTemplate(CELL, 225, (WyckoffSite("a", FracVector([1]), "A"),), (dummy("A"),))
...
def test_exact_asu_path_and_expansion() -> None:
    ...
    assert FundamentalDomainTemplateView(asu, kind="structure") == prototype
    with pytest.raises(TypeError):
        FundamentalDomainTemplateView(asu, kind="bogus")
...
def test_prototype_view_rewrap_rejects_arguments() -> None:
    view = FundamentalDomainTemplateView(_rocksalt_asu())
    with pytest.raises(ValueError):
        FundamentalDomainTemplateView(view, tolerance=0.123)
    with pytest.raises(ValueError):
        FundamentalDomainTemplateView(view, setting="bogus")
    with pytest.raises(ValueError):
        FundamentalDomainTemplateView(view, kind="bogus")
...
def test_anonymous_prototype_source_uses_original_labels() -> None:
    ...
    prototype = FundamentalDomainTemplateView(anonymous)
...
def test_recognition_path_is_spglib_gated() -> None:
    ...
    assert FundamentalDomainTemplateView(unitcell) == FundamentalDomainTemplateView(asu)
...
def test_prototype_view_resolves_nested_asu_source_once_across_value_operations() -> None:
    ...
    view = FundamentalDomainTemplateView(asu)
...
def test_prototype_view_retains_tolerance_and_denominator_and_resolves_source_once(monkeypatch) -> None:
    ...
    view = FundamentalDomainTemplateView(source, tolerance=0.125, limit_denominator=17)
    assert source.resolve_calls == 0
    _ = view.spacegroup
    assert source.resolve_calls == 1
...
def test_prototype_view_unsupported_data_fails_atomically_on_first_access() -> None:
    ...
    view = FundamentalDomainTemplateView(mixed)
...
def test_prototype_view_pickle_preserves_unresolved_and_resolved_states() -> None:
    source = CountingStructureResolver(_rocksalt_unitcell())
    unresolved = FundamentalDomainTemplateView(ASUStructureView(source, setting=Spacegroup.standard(1)))
    ...
    source = CountingStructureResolver(_rocksalt_unitcell())
    resolved = FundamentalDomainTemplateView(ASUStructureView(source, setting=Spacegroup.standard(1)))
...
def test_prototype_view_native_unview_preserves_identity() -> None:
    native = FundamentalDomainTemplate(
        CELL,
        225,
        (WyckoffSite("a", EMPTY, "A"),),
        (dummy("A"),),
    )
    assert FundamentalDomainTemplateView(native).unview() is native
...
def test_prototype_datastream_path_is_not_parsed_at_construction(tmp_path, monkeypatch) -> None:
    ...
    view = FundamentalDomainTemplateView(str(path))
    assert calls == 0
