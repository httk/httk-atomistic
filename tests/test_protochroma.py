"""Tests for the geometry-free, element-free protochroma family."""
...
def _rocksalt_pattern() -> Protochroma:
...
def test_interchangeable_classes_tie_to_the_same_label() -> None:
    first = Protochroma(225, [("e", "A"), ("e", "B")])
    swapped = Protochroma(225, [("e", "B"), ("e", "A")])
...
def test_erasure_from_fundamental_domain_pattern_needs_no_spglib() -> None:
    ...
    assert str(ProtochromaView(fdp).label) == "AB_cF8_225_a_b"
...
def test_erasure_from_structure_via_recognition() -> None:
    ...
    assert str(ProtochromaView(asu).label) == "AB_cF8_225_a_b"


def test_label_string_dispatch() -> None:
    view = ProtochromaView("AB_cF8_225_a_b")
    assert view.unview() == _rocksalt_pattern()
    assert str(ProtochromaLabel("AB_cF8_225_a_b")) == "AB_cF8_225_a_b"


def test_view_is_lazy_and_unwrap_is_identity() -> None:
    proto = Protostructure(225, [("a", "Na"), ("b", "Cl")])
    view = ProtochromaView(proto)
    assert view._resolved_protochroma is None
    assert view.unwrap() is proto
    assert view._resolved_protochroma is None
    _ = view.spacegroup
    assert view._resolved_protochroma is not None
    assert view.unview() == _rocksalt_pattern()


def test_native_protochroma_view_returns_value_identity() -> None:
    pattern = _rocksalt_pattern()
    assert ProtochromaView(pattern).unview() is pattern
    assert ProtochromaView(ProtochromaView(pattern)) is not None


def test_occupation_str_coerces_fields() -> None:
    occupation = ProtochromaOccupation(0, 0)
