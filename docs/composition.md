...
`ChemicalFormula` is a canonical reduced formula, and `Chromaformula` is a
canonical OPTIMADE anonymous formula (`AnonymousFormula` remains an alias).
`CompositionView`, `ChemicalFormulaView`, and `ChromaformulaView` present
...
alphabetical and coefficients are GCD-reduced. `Chromaformula` uses
...
- `ChromaformulaView` can anonymize a complete real-element composition and
...
assert ChromaformulaView({"Al": 2, "O": 3}) == "A3B2"
```
...
for where `ChemicalFormula` and `Chromaformula` sit within the full
