...
| --- | --- | --- |
| Wyckoff positions only | {py:class}`~httk.atomistic.Protochroma` | {py:class}`~httk.atomistic.Protostructure` |
| Geometrical class | {py:class}`~httk.atomistic.Prototype` | {py:class}`~httk.atomistic.Crystallotype` |
| Exact geometry | {py:class}`~httk.atomistic.Chromastructure` | {doc}`Structure <structures>` |
...
- Protochroma + geometrical-class information → Prototype
- Protochroma + species assignment → Protostructure
- Prototype + species assignment → Crystallotype
- Protostructure + geometrical-class information → Crystallotype
- Prototype + exact geometrical parameters → Chromastructure
- Crystallotype + exact parameters → Structure
- Chromastructure + species assignment → Structure

A {py:class}`~httk.atomistic.Prototype` and a
{py:class}`~httk.atomistic.Crystallotype` pin their geometrical class by a
...
exact-structure data**. A prototype or crystallotype therefore corresponds to
*many* exact structures within its class — every realization that shares the
class — whereas a {py:class}`~httk.atomistic.Chromastructure` or a `Structure`
...
`Protochroma`, `Protostructure`, `Prototype`, `Crystallotype`,
`Chromaformula`, and `Chromastructure`. The `Chroma` terms are always
compounded because the bare word "Chroma" — like the bare word "Formula" —
would be ambiguous across contexts; an extra word disambiguates it. The
fundamental-domain compounds `FundamentalDomainPattern` and `ASUPattern`
retain the older "Pattern" element, which there names the same anonymous
coloring that "Chroma" now denotes.

Alternative names survive as aliases for discoverability only; documentation and
new code use the canonical names.

| Alias | Canonical |
| --- | --- |
| `AnonymousStructure`, `AnonymousStructureView`, `AnonymousStructureLike` | `Chromastructure` family |
| `Chromatype` | `Prototype` |
| `AnonymousFormula`, `AnonymousFormulaView` | `Chromaformula` family |

## Dummy species

The two anonymous columns of the exact and geometrical-class rows
(`Chromastructure`, `FundamentalDomainPattern`, and the representative held by a
...
from httk.atomistic.models.chromastructure import dummy_species, is_dummy_species
...
with `labels=("other",)` is not a dummy species. A `Protochroma` and a
...
| --- | --- | --- |
| Structure → `Chromastructure`/`FundamentalDomainPattern` | disorder or partial occupancy; duplicate-element species; assemblies; `chemical_composition`; site moments | species identities become dummy labels; charge, formula metadata, `optimization_type`, `immutable_id`, `last_modified`, molecular metadata, and representatives |
...
| --- | --- | --- |
| `ProtochromaView(Protostructure)` | erased anonymous protochroma | exact erasure of species |
| `ProtochromaView(Prototype or Crystallotype)` | the geometrical class's protochroma | exact extraction/erasure |
| `ProtochromaView(FundamentalDomainPattern)` | folded protochroma | exact discretization |
| `ProtochromaView(unitcell or ordinary structure)` | recognized protochroma | tolerant recognition (spglib) |
| `ChromastructureView(Chromastructure)` | chromastructure view | exact/pass-through |
| `ChromastructureView(FundamentalDomainPattern)` | expanded unit cell | exact |
| `ChromastructureView(structure)` | anonymized projection | validates the rejection rules above |
| `PrototypeView(Prototype)` | prototype view | exact/pass-through |
| `PrototypeView(Crystallotype)` | anonymized prototype (discriminator carried over) | exact; the discriminator names the species-independent class |
...
| `CrystallotypeView(Crystallotype)` | crystallotype view | exact/pass-through |
| `CrystallotypeView(unitcell or ordinary structure)` | recognized crystallotype | tolerant recognition via the conventional-cell path (spglib) |
| `ProtostructureView(Protostructure)` | protostructure view | exact/pass-through |
| `ProtostructureView(Crystallotype)` | protostructure (drops the geometrical class) | exact |
...
| `UnitcellStructureView(Chromastructure or FundamentalDomainPattern)` | — | raises: dummy species are not real species |
| `ProtostructureView(Chromastructure)` | — | raises: dummy species are not real species |
...
ASU, chromastructure, prototype, and protostructure values use exact data, with no
...
```python
from httk.atomistic import Chromastructure, ChromastructureView

pattern = Chromastructure(
    [[5, 0, 0], [0, 5, 0], [0, 0, 5]],
    [[0, 0, 0], [1 / 2, 1 / 2, 1 / 2]],
    species_at_sites=("A", "B"),
)
view = ChromastructureView(pattern)
...
change the key. `Protochroma` is the anonymous counterpart of that key.
...
`httk.atomistic.models.protochroma.notation`. The grammar is:
...
`ProtochromaLabel` plus the `:` species suffix. AFLOW, by contrast, orders the
...
`parse_anonymous_formula` for `Chromaformula`.

### Canonical vs plain labels

Any faithful render of an object is *the* protochroma or protostructure label.
The *canonical* protochroma or protostructure label is the one obtained from a
...
`Crystallotype` (their `discriminator` field), which name a species-independent
...
| --- | --- | --- |
| `ProtochromaRecord` | `atomistic_protochroma` | `Protochroma` |
| `PrototypeRecord` | `atomistic_prototype` | `Prototype` |
| `CrystallotypeRecord` | `atomistic_crystallotype` | `Crystallotype` |
...
values differ. `PrototypeRecord` and `CrystallotypeRecord` enforce the
"at least one of representative or discriminator" rule and store the optional
representative as a nested record. `Chromastructure` itself stays non-storable,
as before.
...
`CrystallotypeRecord` exposes the same label column; its discriminator is not
part of the label, so crystallotypes that share a protostructure but differ in
class collide on it.
...
`CrystallotypeRecord` as the optional representative) and deliberately has no
...
- The `Crystallotype(prototype, species=...)` species-assignment convenience
  constructor.
